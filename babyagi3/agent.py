...
class Agent(EventEmitter):
    ...
    def __init__(self, model: str = None, load_tools: bool = True, config: dict = None, use_litellm: bool = True):
        ...
        # Bounded to 100 entries with LRU eviction.
        self._full_tool_results: OrderedDict[str, str] = OrderedDict()

        # Thread repair flag - only repair after a previous failure
        self._thread_needs_repair: set[str] = set()

        # Configured max_tokens for agent LLM calls
        self._agent_max_tokens = llm_config.agent_model.max_tokens if llm_config else 8096
...
Work autonomously. Use tools as needed. When done, provide a brief summary of what you accomplished."""
    ...
    def run(self, user_input: str, thread_id: str = "main") -> str:
        ...
        """
        return asyncio.run(self.run_async(user_input, thread_id))
    ...
    async def run_async(self, user_input: str, thread_id: str = "main", context: dict = None, stream: bool = False) -> str:
        ...
        async with self._get_thread_lock(thread_id):
            ...
            # Evict oldest inactive threads if over limit
            if len(self.threads) > self.MAX_THREADS:
                # Never evict main or the current thread
                protected = {"main", thread_id}
                # Also protect any thread with an active objective
                protected.update(f"objective_{oid}" for oid in self._running_objectives)
                evictable = [tid for tid in self.threads if tid not in protected]
                # Remove oldest (first inserted) threads
                while len(self.threads) > self.MAX_THREADS and evictable:
                    self.threads.pop(evictable.pop(0), None)

            # Auto-repair corrupted threads only when flagged from previous failure
            if thread_id in self._thread_needs_repair and thread:
                ...
                self._thread_needs_repair.discard(thread_id)
            ...
            while True:
                ...
                try:
                    ...
                    if not self._is_context_overflow(e):
                        raise
                    # Flag thread for repair on next invocation
                    self._thread_needs_repair.add(thread_id)
                ...
                for block in response.content:
                    if block.type == "tool_use":
                        ...
                        if len(result_json) > SUMMARIZE_THRESHOLD_CHARS:
                            ...
                            while len(self._full_tool_results) > 100:
                                self._full_tool_results.popitem(last=False)
...
Work autonomously. When done, provide a summary of what was accomplished and how it was verified.""")
    ...
    async def _track_objective_usage(self, objective_id: str, response):
        ...
        from metrics.costs import calculate_cost
    ...
    async def shutdown(self):
        """Graceful shutdown: cancel objectives, stop scheduler, close DB."""
        logger.info("Agent shutting down...")

        # Cancel running objectives
        for obj_id in list(self._running_objectives):
            token = self._cancellation_tokens.get(obj_id)
            if token:
                token.set()
        ...
        if self.memory is not None:
            try:
                ...
                logger.warning("Error closing memory store: %s", e)
...
"""
            ...
            tool_def = json.loads(result["result"])
