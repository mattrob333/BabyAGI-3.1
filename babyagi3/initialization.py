...
def _task_name_exists(agent, name: str) -> bool:
    """Check if a task with the given name already exists in the scheduler."""
    for task in agent.scheduler.tasks.values():
        if task.name == name:
            return True
    return False
...
def schedule_post_init_tasks(agent):
    ...
    and sets up:
      ...
      3. Email inbox check (5 min after init, then every 60 min) — if email is configured

    Skips tasks that already exist (by name) to prevent duplicates on re-init.
    ...
    if owner_email and not _task_name_exists(agent, "Daily Stats Report"):
    ...
    elif _task_name_exists(agent, "Daily Stats Report"):
        logger.info("Daily Stats Report already exists, skipping")

    # --- Task 2: Daily Self-Improvement ---
    if not _task_name_exists(agent, "Daily Self-Improvement"):
        ...
        logger.info("Daily Self-Improvement already exists, skipping")

    # --- Task 3: Email Check (every 60 minutes) ---
    if (state.get("agentmail_configured") or os.environ.get("AGENTMAIL_API_KEY")) and not _task_name_exists(agent, "Check Email"):
        email_check_anchor = (now + timedelta(minutes=5)).isoformat()
        ...
        logger.info("Scheduled email check every 60 minutes (first run in ~5 minutes)")
    elif _task_name_exists(agent, "Check Email"):
        logger.info("Check Email already exists, skipping")
...
_EMAIL_CHECK_GOAL = """Check the email inbox for new unread messages. DO NOT send any emails or replies unless the message is truly urgent.

Steps:
1. Use the check_email tool to fetch recent unread messages.
2. If there are no new messages, do nothing. Stop immediately.
3. For each unread message:
   a. Read the full message content.
   b. Categorize it as: URGENT, IMPORTANT, ROUTINE, or SPAM/NEWSLETTER.
   c. Store a brief summary in memory with the category tag.
   d. Mark the message as read.
4. ONLY if a message is categorized as URGENT (e.g., time-sensitive request from the owner, critical business matter):
   - Send ONE brief notification email to the owner with subject "[URGENT] ..." summarizing what needs attention.
5. For all other categories: just log them in memory. Do NOT send emails, replies, or notifications.

IMPORTANT RULES:
- NEVER reply to external emails on the owner's behalf without explicit instruction.
- NEVER send summary emails for routine inbox checks.
- NEVER forward or CC emails without being asked.
- Be silent and efficient. The owner will check the dashboard if they want details."""
...
def _get_agent_email() -> str:
    """Get the agent's email address from AgentMail, if configured."""
    if not os.environ.get("AGENTMAIL_API_KEY"):
        return ""
    try:
        from utils.email_client import get_inbox_id
        return get_inbox_id() or ""
    except Exception:
        return ""
