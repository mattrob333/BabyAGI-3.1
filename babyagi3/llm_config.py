...
- Multi-provider support (OpenAI, Anthropic, Moonshot) via LiteLLM
...
- If MOONSHOT_API_KEY is set: Uses Kimi models by default
- If multiple are set: Prefers Anthropic > Moonshot > OpenAI (can be overridden in config)
- If none is set: Raises helpful error at startup
"""
...
class ModelConfig:
    ...
    provider: str = "auto"  # auto, openai, anthropic, moonshot
    ...
    def get_litellm_model(self) -> str:
        ...
        # moonshot/ prefix for Moonshot/Kimi models
        ...
        elif self.model_id.startswith("moonshot/") or self.model_id.startswith("kimi"):
            # Ensure moonshot/ prefix for LiteLLM
            if not self.model_id.startswith("moonshot/"):
...
def get_available_provider() -> str:
    ...
    if _get_api_key("ANTHROPIC_API_KEY"):
        return "anthropic"
    elif _get_api_key("MOONSHOT_API_KEY"):
        return "moonshot"
...
def is_llm_configured() -> bool:
    ...
    """
    return bool(_get_api_key("OPENAI_API_KEY") or _get_api_key("ANTHROPIC_API_KEY") or _get_api_key("MOONSHOT_API_KEY"))
...
║                                                                              ║
║  Option 2: Moonshot AI (Kimi K2.5)                                            ║
║  ──────────────────────────────────                                          ║
...
# Default models for Moonshot/Kimi (when MOONSHOT_API_KEY is set)
MOONSHOT_DEFAULTS = {
    "skill_building": "moonshot/kimi-k2.5",  # Most powerful Kimi model
    "coding": "moonshot/kimi-k2.5",           # Best for code generation
    "research": "moonshot/kimi-k2.5",         # Good for research/analysis
    "agent": "moonshot/kimi-k2.5",            # Main agent operations
    "memory": "moonshot/kimi-k2.5",           # Memory extraction/summaries
    "fast": "moonshot/kimi-k2.5",             # Quick operations
...
class LLMConfig:
    ...
    - agent: Main agent operations (e.g., Claude Sonnet, GPT-4o, Kimi K2.5)
    - memory: Memory operations like extraction, summaries (e.g., Claude Sonnet, GPT-4o-mini, Kimi K2.5)
    - fast: Quick operations like classification (e.g., Claude Haiku, GPT-4o-mini, Kimi K2.5)
    """
    ...
    def from_config(cls, config: dict) -> "LLMConfig":
        ...
        # Allow AGENT_MODEL env var to override ALL use cases (same as create_default_config).
        # This ensures the user's model choice is respected even when config.yaml is loaded.
        agent_model_override = _get_api_key("AGENT_MODEL")
        if agent_model_override:
            instance.skill_building_model = ModelConfig(model_id=agent_model_override, max_tokens=16384)
            instance.coding_model = ModelConfig(model_id=agent_model_override, max_tokens=8096)
            instance.research_model = ModelConfig(model_id=agent_model_override, max_tokens=4096)
            instance.agent_model = ModelConfig(model_id=agent_model_override, max_tokens=8096)
            instance.memory_model = ModelConfig(model_id=agent_model_override, max_tokens=4096)
            instance.fast_model = ModelConfig(model_id=agent_model_override, max_tokens=1024)
...
def create_default_config() -> LLMConfig:
    ...
    # Allow AGENT_MODEL env var to override ALL model use cases.
    # This is needed because Agent() may be created before config.yaml is loaded,
    # and when users switch models they want everything to use the new model.
    agent_model_override = _get_api_key("AGENT_MODEL")
    if agent_model_override:
        for key in ["skill_building", "coding", "research", "agent", "memory", "fast"]:
            models[key] = agent_model_override
    ...
    if provider == "openai":
        ...
        # Moonshot doesn't provide embeddings; use local
        embedding_model = "local"
    else:
        embedding_model = "local"
