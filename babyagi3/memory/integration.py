...
def create_enhanced_memory_tool(memory):
    ...
    def fn(params: dict, agent) -> dict:
        ...
        if action == "store":
            ...
            # Store multiple items in one call (much faster than repeated store calls)
            items = params.get("items", [])
            if not items:
                return {"error": "items list is required for batch_store"}
            results = []
            for item in items:
                ...
                results.append({"stored": True, "event_id": event.id})
...
Memory Actions:
- store: Store a single fact or observation (content)
- batch_store: Store MULTIPLE items at once (items: list of strings). ALWAYS prefer this over multiple store calls.
...
- memory(action="batch_store", items=["John works at Acme Corp", "Dentist appointment Tuesday 2pm", "Buy milk, eggs, bread"])
...
- memory(action="search_learnings", query="email formatting")""",
        parameters={
            ...
            "properties": {
                "action": {
                    "type": "string",
                    "enum": ["store", "batch_store", "search", "list", "find_entity", "find_relationship", "get_summary", "get_context", "deep_search", "list_tools", "tool_stats", "problematic_tools", "get_tool", "list_learnings", "learning_stats", "get_preferences", "search_learnings", "refresh_preferences"],
                },
                "content": {"type": "string", "description": "Content to store (for store action)"},
                "items": {"type": "array", "items": {"type": "string"}, "description": "List of items to store (for batch_store action)"},
