...
def parse_datetime(s: str | None) -> datetime | None:
    """Parse ISO datetime string, always returning a naive (no-tzinfo) datetime.

    The codebase uses naive datetimes throughout (implicitly UTC).
    Stored values may have timezone suffixes from earlier runs,
    so we strip tzinfo to avoid naive-vs-aware comparison errors.
    """
    if s is None:
        return None
...
class MemoryStore:
    ...
    def transaction(self):
        """Context manager for batched writes within a single transaction."""
