...
def get_llm_client(
    ...
    try:
        ...
        # Fallback if llm_config is not available — check env var first
        import os
        fallback = os.environ.get("AGENT_MODEL", "claude-sonnet-4-20250514")
        return InstrumentedLiteLLM(default_model=fallback)
...
def get_model_for_use_case(
    ...
    try:
        ...
        # Fallback — use AGENT_MODEL env var if set
        import os
        fallback = os.environ.get("AGENT_MODEL", "claude-sonnet-4-20250514")
        return fallback
...
class _AnthropicStyleMessages:
    ...
    def create(self, **kwargs) -> _AnthropicStyleResponse:
        ...
        call_kwargs = {
            ...
            "max_tokens": max_tokens,
            "timeout": 120,  # 120s timeout to prevent backend hangs
        }

        if converted_tools:
            call_kwargs["tools"] = converted_tools

        # Disable thinking mode for Moonshot/Kimi models (use Instant mode)
        if model and (model.startswith("moonshot/") or model.startswith("kimi")):
