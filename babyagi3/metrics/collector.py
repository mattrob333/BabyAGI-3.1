...
class MetricsCollector:
    ...
    # Maximum number of in-memory metric entries per list
    MAX_METRICS = 10000
    ...
    def _on_llm_call_end(self, event: dict):
        ...
        if len(self._llm_calls) > self.MAX_METRICS:
            self._llm_calls = self._llm_calls[-self.MAX_METRICS:]
    ...
    def _on_embedding_call_end(self, event: dict):
        ...
        if len(self._embedding_calls) > self.MAX_METRICS:
            self._embedding_calls = self._embedding_calls[-self.MAX_METRICS:]
    ...
    def _on_tool_end(self, event: dict):
        ...
        if len(self._tool_calls) > self.MAX_METRICS:
            self._tool_calls = self._tool_calls[-self.MAX_METRICS:]
