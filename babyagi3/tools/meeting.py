...
class MeetingProcessor:
    ...
    @property
    def client(self):
        """Get instrumented LLM client (supports any provider via LiteLLM)."""
        if self._client is None:
            from metrics import LiteLLMAnthropicAdapter
...
What's the most useful insight to share right now? (Or "---" if nothing notable)"""
        ...
        try:
            ...
            with track_source("meeting_backchannel"):
                from metrics import get_model_for_use_case
                response = self.client.messages.create(
                    model=get_model_for_use_case("fast"),
                    max_tokens=200,
...
Extract structured information from this meeting."""
        ...
        try:
            ...
            with track_source("meeting_summary"):
                from metrics import get_model_for_use_case
                response = self.client.messages.create(
                    model=get_model_for_use_case("research"),
                    max_tokens=4096,
                    system=system_prompt,
                    messages=[{"role": "user", "content": user_message}],
                )
