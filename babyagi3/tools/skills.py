    ...
    def fn(params: dict, ag: "Agent") -> dict:
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            try:
                            ...
                            if ag.memory:
                                ...
                                for cred in local_conns:
                                    if cred.metadata and cred.metadata.get("connection_id") == acc_id:
                                        redirect_url = cred.metadata.get("redirect_url")
                                        break
                            msg = f"A pending connection already exists for {app}."
                            if redirect_url:
                                msg += f"\n\nClick here to authorize: [Authorize {app}]({redirect_url})\n\nLet me know once you've completed the authorization."
                            else:
                                msg += " Please complete the authorization, then let me know."
                            return {
                                "status": "pending",
                                "app": app,
                                "connection_id": acc_id,
                                ...
                                "IMPORTANT_FOR_ASSISTANT": "Present the redirect_url as a clickable markdown link to the user. Use the exact format: [Authorize APP_NAME](url)."
                            }
                ...
                return {
                    ...
                    "redirect_url": redirect_url,
                    "user_id": user_id,
                    "message": f"Click the link below to authorize {app}:\n\n[Authorize {app}]({redirect_url})\n\nOnce you've completed the authorization, let me know and I'll verify the connection.",
                    "next_step": f"After authorizing, use: composio_setup(action='poll_connection', app='{app}', connection_id='{conn_id}')",
                    "IMPORTANT_FOR_ASSISTANT": "Present the redirect_url as a clickable markdown link to the user. Use the exact format: [Authorize APP_NAME](url). Do NOT show the raw URL alone."
                }
            except Exception as e:
                error_msg = str(e)
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            try:
                ...
                if needs_reauth:
                    links = "\n".join(
                        f"- [Re-authorize {app or 'connection'}]({r['redirect_url']})"
                        for r in needs_reauth if r.get("redirect_url")
                    )
                    return {
                        "status": "reauth_required",
                        "app": app or "unknown",
                        "refreshed": refreshed,
                        "message": f"OAuth tokens expired. Click below to re-authorize:\n\n{links}\n\nLet me know once you've completed the re-authorization.",
                        "next_step": "After re-authorizing, use: composio_setup(action='poll_connection', ...)",
                        "IMPORTANT_FOR_ASSISTANT": "Present the redirect URLs as clickable markdown links."
                    }
