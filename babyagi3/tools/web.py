...
- web_search: Smart search with fallback chain (Exa > Brave > DuckDuckGo)
...
def _search_exa(query: str, max_results: int) -> dict | None:
    """Try Exa semantic search. Returns normalized results or None on failure."""
    ...
    try:
        resp = request_json(
            ...
            json_body={"query": query, "numResults": max_results, "type": "neural"},
        )
        if "error" in resp:
            logger.debug("Exa search failed: %s", resp.get("error"))
            return None

        raw_results = resp.get("data", {})
        # Exa returns results under a "results" key
        items = raw_results.get("results", []) if isinstance(raw_results, dict) else []
        return {
            "query": query,
            "provider": "exa",
            "results": [
        ...
        logger.debug("Exa search error: %s", e)
        return None


def _search_brave(query: str, max_results: int) -> dict | None:
    """Try Brave Search API. Returns normalized results or None on failure."""
    ...
    try:
        ...
        if "error" in resp:
            logger.debug("Brave search failed: %s", resp.get("error"))
            return None

        raw_data = resp.get("data", {})
        web_results = raw_data.get("web", {}).get("results", []) if isinstance(raw_data, dict) else []
        return {
            "query": query,
            "provider": "brave",
            "results": [
...
def _search_ddg(query: str, max_results: int) -> dict:
    """DuckDuckGo fallback search. Always available, no API key needed."""
    try:
...
def web_search(query: str, max_results: int = 5) -> dict:
    ...
    Automatically selects the best search backend based on configured API keys:
    1. Exa (semantic/neural search) if EXA_API_KEY is set
    ...
    for search_fn in (_search_exa, _search_brave):
        result = search_fn(query, max_results)
        if result is not None:
            logger.debug("web_search using provider: %s", result.get("provider"))
            return result

    # DuckDuckGo is the guaranteed fallback
